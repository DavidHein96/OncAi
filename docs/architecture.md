...
| STATIC | `fc_adjudications` | Cross-batch adjudication rounds (`<round>/<round>.adjudication_pkg.json` + `.adjudications.jsonl`) become `extractions_adjudicated.<round>`. Optional `<round>/<round>.sql` sidecars reshape adjudicated rows into `extractions_gold`. |
