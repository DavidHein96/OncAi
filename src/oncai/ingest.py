...
# --- STATIC: fc_adjudications -------------------------------------------


def _ingest_fc_adjudications(
    config: OncaiConfig,
    *,
    dry_run: bool = False,
    forgotten_targets: set[str] | None = None,
    ...
    for round_dir in sorted(p for p in inbox_dir.iterdir() if p.is_dir()):
        round_name = round_dir.name
        if round_name in forgotten:
            result.notes.append(f"{round_name}: skipped because tombstoned")
            continue
        package_files = sorted(round_dir.glob(f"*{ADJUDICATION_PACKAGE_SUFFIX}"))
        log_files = sorted(round_dir.glob(f"*{ADJUDICATION_LOG_SUFFIX}"))
        descriptor = round_dir / ADJUDICATION_DESCRIPTOR
        files = [*package_files, *log_files]
...
def _dispatch(
    ...
    *,
    ...
    if folder == "fc_adjudications":
        return _ingest_fc_adjudications(
            config, dry_run=dry_run, forgotten_targets=forgotten_targets
        )
    if folder == "cohorts":
