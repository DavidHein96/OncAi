...
# to them without doing CREATE SCHEMA themselves.
TRANSFORM_SCHEMAS: set[str] = {"extractions_transformed"}

# Staging schemas — populated only by ``oncai fc stage`` for ad-hoc exploration
# of un-curated runs. Auto-created so users can land staged tables without
# CREATE SCHEMA boilerplate.
STAGING_SCHEMAS: set[str] = {"extractions_staging"}
...
def update_database_folder(config: OncaiConfig, folder: str) -> dict[str, int]:
    ...
    try:
        ...
        # ``oncai fc stage``. Both auto-created so callers don't need their
        ...
        if folder in MULTI_TABLE_FOLDERS:
            for parquet_path in parquets:
                ...
                try:
                    ...
                    _run_sibling_sql(con, parquet_path)
            ...
            try:
                ...
                results[full_name] = _count_rows(con, qname)
                _run_sibling_sql(con, lake_folder / f"{folder}.parquet")
            except Exception as e:
                print(f"Error creating table {full_name}: {e}")
    finally:
        con.close()
...
def build_database(config: OncaiConfig, force: bool = False) -> dict[str, int]:
    """
    ...
    - runs: run log
    - extractions_transformed / extractions_staging: created empty for
      per-base ``.sql`` transforms and ``oncai fc stage`` to write into.
    ...
    # Create all schemas upfront — including transform/staging schemas so per-base
    # .sql files and ``oncai fc stage`` can write into them without their own
    # CREATE SCHEMA boilerplate.
...
def _run_sibling_sql(con: duckdb.DuckDBPyConnection, parquet_path: Path) -> None:
    ...
    curated run is self-contained: drop the JSONL + manifest + ``.sql``
    together and ``oncai build-db`` materialises both the raw table and
    any derived tables/views the SQL declares. Errors print but don't
    abort the build — one bad transform shouldn't tank the whole rebuild.
    """
    ...
    try:
        ...
        print(f"  Error running {sql_path.name}: {e}")
