...
SUPPORTED_TOMBSTONE_KINDS = frozenset(
    {"fc_extractions", "fc_reviews", "fc_adjudications", "cohorts"}
)
...
def inbox_source_paths_for_target(
    ...
    if kind in {"fc_extractions", "fc_reviews", "fc_adjudications"}:
        return [inbox / target]
    if kind == "cohorts":
...
def inbox_targets_for_kind(config: OncaiConfig, kind: str) -> set[str]:
    ...
    if kind in {"fc_reviews", "fc_adjudications"}:
        return {
            path.name
            for path in inbox.iterdir()
            if path.is_dir()
            and (
                ...
                or any(path.glob("*.adjudication_pkg.json"))
                or any(path.glob("*.adjudications.jsonl"))
            )
        }
    if kind == "cohorts":
