...
│ inbox/fc_adjudications/│         │                  │         │                  │         extractions_raw,
│ inbox/runs/            │         └──────────────────┘         └──────────────────┘         extractions_adjudicated,
...
- `inbox/fc_adjudications/<round>/<round>.adjudication_pkg.json` + `<round>.adjudications.jsonl` → adjudication rounds comparing two extraction batches. Package creation is implemented; the adjudication-log loader is the next slice.
- `inbox/tombstones/*.tombstone.json` → `oncai ingest tombstones` (append-only forget/revive events; active forgets are honored by `ingest` for `fc_extractions`, `fc_reviews`, `fc_adjudications`, and `cohorts`).
...
| `extractions_adjudicated` | `fc_adjudications` | One table per completed cross-batch adjudication round |
